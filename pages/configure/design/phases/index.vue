  ...
  <div class="flex flex-col">
    ...
    >
      ...
      <div
        ...
        :key="phase.phase_id"
      >
        <div class="w-11/12">{{ phase.phase }}</div>
        <div class="w-1/12 flex flex-col flex-end">
          <!--Dropdown menu-->
          <div class="relative inline-block text-left">
            <div>
              <div
                @click="gridOption('grid-data-' + phase.phase_id)"
            ...
            <div
              ...
              :id="'grid-data-' + phase.phase_id"
            >
              <div class="py-1">
                <nuxt-link
                  :to="`/configure/design/phases/${phase.phase_id}`"
                  class="
                    text-gray-700
                    block
                    px-4
                ...
                <nuxt-link
                  :to="`/configure/design/phases/${phase.phase_id}/edit`"
                  class="
                    text-gray-700
                    block
                    px-4
                ...
                <div
                  @click="deletePhase(phase.phase_id)"
                  class="
                    text-gray-700
                    block
                    px-4
...
export default {
  ...
  methods: {
    create() {
      this.$router.push('/configure/design/phases/create')
    },
    filter() {
    ...
    edit(phase_id) {
      ...
      this.$router.push('/configure/design/phases/' + { phase_id })
    },
    deletePhase(phase_id) {
      destroy(this.$axios, 'phase/' + phase_id + '/')
        .then((results) => {})
        .catch((error) => {
          console.log(error)
        })
  ...
  async fetch() {
    get(this.$axios, 'phase/')
      .then((results) => {
        this.phases = results.results
